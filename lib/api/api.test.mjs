...
t('public intake reports UTF-8 byte size instead of UTF-16 length', () => {
  const body = '🚑'.repeat(4);
  const submission = buildPublicIntakeSubmission(body, body, 'text/plain');
  assert.equal(body.length, 8);
  assert.equal(utf8ByteLength(body), 16);
  assert.equal(submission.payloadSizeChars, 16);
});

await at('public intake rejects byte-heavy payloads without content-length', async () => {
  const body = '界'.repeat(Math.floor(MAX_PUBLIC_INTAKE_BODY_BYTES / 3) + 1);
  assert.equal(body.length < MAX_PUBLIC_INTAKE_BODY_BYTES, true);
  assert.equal(utf8ByteLength(body) > MAX_PUBLIC_INTAKE_BODY_BYTES, true);
  const parsed = await readPublicIntakePayload(new Request('https://example.test/api/v1/public-intake', {
    method: 'POST',
    headers: { 'content-type': 'text/plain' },
    body,
  }));
  assert.deepEqual(parsed, { ok: false, error: 'payload_too_large' });
});
