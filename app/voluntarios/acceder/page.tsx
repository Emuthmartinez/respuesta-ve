...
const NEXT = '/voluntarios';

const STR = {
  es: {
    heading: 'Acceso para voluntarios',
    desc: 'Inicia sesión o crea una cuenta. Solo voluntarios y responders usan esta sección — para reportar daños no necesitas cuenta.',
    ...
    working: 'Un momento…',
    magic: '¿Prefieres un enlace por correo? Envíamelo',
    magicSent: (e: string) => (
      <>Te enviamos un enlace a <strong>{e}</strong>. Ábrelo para entrar.</>
    ),
    confirmSent: (e: string) => (
      <>Cuenta creada. Revisa <strong>{e}</strong> para confirmarla, luego inicia sesión.</>
    ),
    back: 'Volver al mapa',
  ...
  en: {
    ...
    desc: "Sign in or create an account. Only volunteers and responders use this section — no account needed to report damage.",
...
export default function AccederPage() {
  ...
  async function withPassword(e: React.SyntheticEvent) {
    ...
    if (mode === 'signup') {
      ...
      if (data.session) window.location.href = NEXT; // email confirmation disabled → signed in
      else setNotice(s.confirmSent(email)); // confirmation required → email sent
    } else {
  ...
  return (
    <div className="mx-auto max-w-md px-4 py-12">
      ...
      <p className="mt-6 text-xs text-zinc-500">
        <Link href="/" className="underline">{s.back}</Link>
      </p>
    </div>
  );
}
