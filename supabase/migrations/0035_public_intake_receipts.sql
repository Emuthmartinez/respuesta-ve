-- =====================================================================
-- 0035 — Public intake receipt polling + larger typed-file payloads.
--
-- Public callers get a receipt id and can poll a receipt-safe status snapshot.
-- Raw payloads/contact/notes stay restricted. Processors/operators can later
-- mark the queue row as triaged/promoted and attach canonical record pointers.
-- =====================================================================

alter table public.public_data_intake_submissions
  drop constraint if exists public_data_intake_submissions_payload_size_chars_check,
  add constraint public_data_intake_submissions_payload_size_chars_check
    check (payload_size_chars > 0 and payload_size_chars <= 5242880),
  add column if not exists processed_record_kind text,
  add column if not exists processed_record_id text,
  add column if not exists processed_record_url text,
  add column if not exists processed_at timestamptz,
  add column if not exists public_review_note text;

alter table public.public_data_intake_submissions
  drop constraint if exists public_data_intake_processed_kind_check,
  add constraint public_data_intake_processed_kind_check check (
    processed_record_kind is null
    or processed_record_kind in ('person','entity','need','status','media','map_report','other')
  );

alter table public.public_data_intake_submissions
  drop constraint if exists public_data_intake_processed_url_check,
  add constraint public_data_intake_processed_url_check check (
    processed_record_url is null or processed_record_url ~* '^https?://'
  );
