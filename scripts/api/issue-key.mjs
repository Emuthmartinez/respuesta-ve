...
// Usage: node scripts/api/issue-key.mjs "<name>" [--scopes a,b] [--source other] [--per-min N] [--per-day N]
import crypto from 'node:crypto';

const args = process.argv.slice(2);
const name = args.find((a) => !a.startsWith('--'));
if (!name) { console.error('usage: issue-key.mjs "<partner name>" [--scopes score,match,search,ingest] [--source other] [--per-min 60] [--per-day 5000]'); process.exit(1); }
...
const sql =
  `insert into public.partner_api_keys (name, key_hash, key_prefix, scopes, rate_limit_per_min, rate_limit_per_day, ingest_source)\n` +
  `values ('${esc(name)}', '${hash}', '${prefix}', '{${scopes.join(',')}}', ${perMin}, ${perDay}, '${esc(source)}');`;
...
console.log('Scopes:', scopes.join(', '), '| source:', source, '| limits:', `${perMin}/min`, `${perDay}/day`);
